...
export default function Expenses() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ type: 'expense', limit: 100 })
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">{total} registros · Total exibido: <strong>{formatCurrencyBRL(totalAmount)}</strong></p>
        </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {item.category_name && <span>🏷️ {item.category_name}</span>}
